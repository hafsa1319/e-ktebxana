$("body").attr("Page","TeacherDashboard")
         ...
         <div class="col-12">\
            <div class="DashArea">\
                  <div class="Istatistic">\
                     <div class="ChartArea"><canvas id="ReadChart"></canvas></div>\
                     <div class="ChartArea"><canvas id="ListenChart"></canvas></div>\
                  </div>\
                  <div class="StudentsBar">\
                     <div class="BtnAll Active">'+defaultLang(2035)+'</div>\
                     <div class="List"></div>\
                  </div>\
               </div>\
            </div>\
         ...
         var WH = parseInt($(window).height())-340
         $('[Page="TeacherDashboard"] .DashArea .StudentsBar>.List').css({"height":WH+"px"})
         if(WW<980){
            $(".Header .Left").html("<a href='/students' class='BtnStudentList'><i></i> <span> "+defaultLang(2516)+" </span></a>")
         }
         ...
         if(WW<600){
            ...
            ReadChart.canvas.parentNode.style.width = '50%';
         }
         var ListenLabels = [];
         ...
         if(WW<600){
            ...
            ListenChart.canvas.parentNode.style.width = '50%';
         }
         ...
         success: function(data) {
            $.each( data.list, function( i, itemx ) {
                  ...
                  var xx_SCHOOLROOM     =  ""
                  $.each( data.schoolroom, function( ii, itemxx ) {
                        if(xx_SCHOOLROOM_ID==itemxx.id){
                           xx_SCHOOLROOM = itemxx.name
                        }
                  })
               ...
               $('[T="Read"] .Text>span').html(data.statistic.read)
               $('[T="Listen"] .Text>span').html(data.statistic.listen)
               $('[T="Paper"] .Text>span').html(data.statistic.lastPage)
               $('[T="Minute"] .Text>span').html(data.statistic.listenTime)
            
         }
         });

         $(document).on("click",".StudentsBar>.List>.Item",function(){
            var StudentId     =  $(this).attr("studentId")

            $(".BtnAll").removeClass("Active")
            $(".StudentsBar>.List>.Item").removeClass("Active")
            ...
            success: function(data) {
               ...
               $('.Istatistic').html('<div class="ChartArea"><canvas id="ReadChart"></canvas></div><div class="ChartArea"><canvas id="ListenChart"></canvas></div>')
               var ReadLabels = [];
               var ReadData = {labels: ReadLabels,datasets: [{label: defaultLang(2036),data: [],borderColor: "#f00",backgroundColor: ["#ff000054"],}]};
               var ReadConfig = {type: 'bar',data: ReadData,options: {responsive: true,plugins: {legend: {position: 'top',},title: {display: true,text: defaultLang(2037)}}},};
               var ReadChart = new Chart(document.getElementById('ReadChart'),ReadConfig);
         ...
         $(document).on("click",".StudentsBar>.BtnAll",function(){
            var StudentId     =  $(this).attr("studentId")
